...
{
	public static class BinarySearchTreePrinter
	{
		...
		public static void Print<T>(this Node<T> root, int topMargin = 2, int leftMargin = 2) where T : IComparable
		{
			if (root == null) return;
			...
			{
				var item = new NodeInfo<T> {Node = next, Text = $" {next.element} "};
				if (level < last.Count)
				{
